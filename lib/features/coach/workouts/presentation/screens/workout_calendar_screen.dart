...
import '../../../../../features/shared/widgets/bottom_nav_bar.dart' show bottomNavIconColor;

/// Provider for workout assignments
final workoutAssignmentsProvider = FutureProvider<List<WorkoutAssignment>>((ref) async {
  ...
  // Get all clients first, then get their assignments
  final clientRepo = ref.watch(clientRepositoryProvider);
  final clients = await clientRepo.getClients(user.id);
  
  final allAssignments = <WorkoutAssignment>[];
  for (final client in clients) {
    final assignments = await workoutRepo.getAssignedWorkouts(client.id);
...
class _WorkoutCalendarScreenState
  ...
  Widget build(BuildContext context) {
    final assignmentsAsync = ref.watch(workoutAssignmentsProvider);
